...
const DuckNav = ({ siteUrl }) => {
    ...
    return (_jsxs(Navbar, { maxWidth: "xl", position: "sticky", isMenuOpen: isMenuOpen, onMenuOpenChange: setIsMenuOpen, children: [_jsxs(NavbarContent, { className: "basis-1/5", justify: "start", children: [_jsx(NavbarBrand, { as: "li", className: "h-full relative", children: _jsx(SelectProject, { projects: config.projects, currentProject: currentProject }) }), _jsx(NavbarItem, { className: "hidden md:flex ml-2", children: _jsx("ul", { className: "flex gap-4 justify-start", children: currentProject?.children?.map((item) => (_jsx(NavbarItem, { isActive: item.path === pathname, className: "px-0", children: _jsx(NavLink, { href: item.path, children: item.label }) }, item.path))) }) })] }), _jsxs(NavbarContent, { className: "basis-1 pl-4", justify: "end", children: [_jsx(Link, { isExternal: true, "aria-label": "Support Project", className: "text-default-500", href: config.links.sponsor, children: _jsx(HeartFilledIcon, { className: "text-danger" }) }), _jsx(Link, { isExternal: true, "aria-label": "Telegram Chat", className: "text-default-500", href: config.links.telegram, children: _jsx(FaTelegram, { size: 20 }) }), _jsx(ThemeSwitch, {}), _jsx(NavbarItem, { className: "hidden md:flex", style: {
                            colorScheme: "normal",
                        }, children: _jsx(AuthButton, {}) }), _jsx(NavbarMenuToggle, { className: "md:hidden" })] }), _jsx(NavbarMenu, { children: _jsxs("ul", { className: "mx-4 mt-2 flex flex-col gap-2", children: [_jsxs(NavbarItem, { style: {
                                colorScheme: "normal",
                            }, children: [_jsx(AuthButton, { isFull: true }), _jsx(Divider, { className: "my-4" })] }), currentProject?.children?.map((item) => (_jsx(NavbarMenuItem, { isActive: item.path === pathname, children: _jsx(NavLink, { href: item.path, onPress: () => setIsMenuOpen((prev) => !prev), children: item.label }) }, `${item.label}`)))] }) })] }));
};
